  ...
  <tr class="order-list__item">
    <td class="order-list__item__cell {{ css_order_list_cell_nested_line_parent_class }}">
      <table height="100%">
        ...
        <td class="order-list__product-description-cell">
          {% comment %} CUSTOM: Show variant name as main title {% endcomment %}
          {% if line.variant.title != blank and line.variant.title != 'Default Title' %}
            {% assign line_title = line.variant.title %}
          {% elsif line.presentment_title %}
            {% assign line_title = line.presentment_title %}
          {% elsif line.title %}
            {% assign line_title = line.title %}
          {% else %}
          ...
          <span class="order-list__item-title">{{ line_title }}&nbsp;&times;&nbsp;{{ line.quantity }}</span><br/>
